...
OP = "TRFMC_ENGINEERING_CONTENT_ENRICHMENT_SECTION_MARKERS_V49R2"
...
print("=" * 60)
print(OP)
print("section-aware marker hardening · alias normalization · no main.tsx mutation")
print("=" * 60)

for p in [MAIN, V42, ENRICH, CSS]:
    if not p.exists():
        raise SystemExit(f"ERRORE: file mancante: {p}")

main_src = MAIN.read_text(encoding="utf-8")
v42_src = V42.read_text(encoding="utf-8")
enrich_before = ENRICH.read_text(encoding="utf-8")
...
if "EngineeringContentEnrichmentV49" not in v42_src:
    raise SystemExit("ERRORE: V42 non monta EngineeringContentEnrichmentV49")

if "data-trfmc-v49-engineering-enrichment" not in enrich_before:
    raise SystemExit("ERRORE: marker V49 base non presente")

(RDIR / f"EngineeringContentEnrichmentV49.tsx.before_v49r2_{TS}").write_text(enrich_before, encoding="utf-8")
(RDIR / f"styles.css.before_v49r2_{TS}").write_text(css_before, encoding="utf-8")

pre_freeze = ROOT / f"runtime/freezes/TRFMC_BEFORE_ENGINEERING_CONTENT_ENRICHMENT_SECTION_MARKERS_V49R2_{TS}.tar.gz"
subprocess.run([
...
css = css_before
if "TRFMC V49R2 SECTION MARKER HARDENING" not in css:
    css += r'''

/* === TRFMC V49R2 SECTION MARKER HARDENING === */
.v49-section-machine-marker{
  display:inline-flex;
  align-items:center;
  width:max-content;
  max-width:100%;
  margin:0 0 10px;
  padding:5px 9px;
  border:1px solid rgba(120,255,190,.26);
  border-radius:999px;
  background:rgba(120,255,190,.075);
  color:rgba(120,255,190,.90);
  font-size:.68rem;
  font-weight:900;
  letter-spacing:.12em;
...
for marker in [
    "V49_SECTION_MISSION_OVERVIEW",
    "V49_SECTION_VISUAL_ASSETS",
    "V49_SECTION_SCENARIO_KNOWLEDGE",
    "V49_SECTION_NAVIGATION_ARCHITECTURE",
    "V49_SECTION_COMMAND_CENTER",
    "V49_SECTION_DYNAMIC_SCENARIOS",
    "V49_SECTION_FULL_ENGINEERING_STACK",
]:
    ok(f"{marker} present", marker in enrich_now)

ok("main.tsx still mounts V42", "<MissionLayoutOrchestratorV42 />" in main_now)
ok("V42 still mounts V49", "EngineeringContentEnrichmentV49" in v42_now)
ok("alias normalizer present", "normalizeSectionV49" in enrich_now)
ok("active section data attribute present", "data-trfmc-v49-active-section" in enrich_now)
ok("normalized section data attribute present", "data-trfmc-v49-normalized-section" in enrich_now)
ok("machine marker data attribute present", "data-trfmc-v49-section-marker" in enrich_now)
ok("V49R2 CSS present", "TRFMC V49R2 SECTION MARKER HARDENING" in css_now)
ok("V46 deeplink preserved", "data-trfmc-v46-deeplink-index" in v42_now)
...
build_log = RDIR / "npm_build_v49r2.log"
...
rollback = RDIR / "rollback_v49r2_section_markers.sh"
rollback.write_text(f"""#!/usr/bin/env bash
set -Eeuo pipefail
cd "{ROOT}"
cp "{RDIR}/EngineeringContentEnrichmentV49.tsx.before_v49r2_{TS}" frontend/src/layout_orchestrator/EngineeringContentEnrichmentV49.tsx
cp "{RDIR}/styles.css.before_v49r2_{TS}" frontend/src/styles.css
echo "Rollback V49R2 completato"
""", encoding="utf-8")
...
manifest = RDIR / "engineering_content_enrichment_section_markers_manifest_v49r2.json"
...
latest_q = ROOT / "runtime/quality/latest_engineering_content_enrichment_section_markers_v49r2"
latest_r = ROOT / "runtime/releases/latest_engineering_content_enrichment_section_markers_v49r2"
