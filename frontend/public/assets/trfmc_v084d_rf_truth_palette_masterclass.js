(function(){
  ...
  function bind(){
    ...
    $("btnBalanced").onclick=()=>{
      setScenario({freq:3500,eirp:46,pathN:3.1,shadow:8,delay:110,kfactor:11,beam:18});
    };

    $("btnNlos").onclick=()=>{
      setScenario({freq:3500,eirp:46,pathN:3.9,shadow:19,delay:420,kfactor:-8,beam:32});
    };

    $("btnTeach").onclick=()=>{
      state.teacher=!state.teacher;
      $("btnTeach").textContent=state.teacher ? "Teacher view ON" : "Teacher view"; document.body.classList.toggle("teacher-active", state.teacher);
      updateAll();
    };
  ...
  function setupCanvas(c,height){
    ...
    const h=Math.max(160,Math.floor((height||rect.height)*dpr));
  ...
  function sampleObstruction(x,y,site){
    let loss=0;
    buildings.forEach(b=>{
      let hits=0;
      for(let i=0;i<=26;i++){
  ...
  function sectorGain(x,y,site){
    ...
    const side=.08*Math.pow(Math.max(0,Math.cos((off-55)*Math.PI/180)),4);
    return 11*(main+side);
  }
  ...
  function rxPower(x,y,site){
    ...
    const obs=sampleObstruction(x,y,site);
    const shadow=state.shadow*(0.35+0.65*Math.abs(Math.sin(9*x+6*y+site.x*13)));
  ...
  function channelAt(x,y){
    ...
    return {powers,best,sinr,ds,evm,coh,score};
  }

  function palette(score,rsrp,sinr,evm){
    /*
      v84D truth palette:
      Red is reserved for degradation, never for "high score".
      Excellent service is cyan/green-white, stable service is green,
      margin is amber, weak/NLOS is blue, critical is red/dark.
    */
    const critical = (rsrp < -112) || (sinr < 0) || (evm > 28) || (score < 18);
    const weak = (rsrp < -105) || (sinr < 5) || (score < 36);
    const edge = (rsrp < -96) || (sinr < 10) || (score < 56);
    const good = (score >= 74 && sinr >= 12 && evm < 10);

    if(critical) return [120,18,32,210];       // red = real degradation
    if(weak)     return [18,82,140,214];       // blue = weak/NLOS
    if(edge)     return [205,158,42,198];      // amber = margin
    if(good)     return [154,238,205,198];     // pale cyan/green = excellent
    return [48,198,126,198];                   // green = useful service
  }

  function drawField(){
    const {ctx,w,h}=setupCanvas(field,620);
    ...
    for(let yy=0;yy<oh;yy++){
      for(let xx=0;xx<ow;xx++){
        const nx=xx/(ow-1),ny=yy/(oh-1);
        const c=channelAt(nx,ny);
        const col=palette(Math.min(96,c.score),c.best.p,c.sinr,c.evm);
        const idx=(yy*ow+xx)*4;
        img.data[idx]=col[0];
    ...
    ctx.filter="blur(2.5px) saturate(.78) brightness(.78) contrast(1.04)";
  ...
  function drawBeam(ctx,w,h){
    sites.forEach((s,idx)=>{
      ...
      const g=ctx.createRadialGradient(sx,sy,10,sx+Math.cos(a)*w*.24,sy+Math.sin(a)*h*.24,w*.44);
      g.addColorStop(0,idx===1?"rgba(66,245,111,.18)":"rgba(143,240,255,.10)");
      g.addColorStop(.50,"rgba(143,240,255,.055)");
      g.addColorStop(1,"rgba(0,0,0,0)");
      ctx.fillStyle=g;
      ctx.beginPath();
      ctx.ellipse(sx,sy,w*.42,h*.16,a,0,Math.PI*2);
      ctx.fill();
      ctx.restore();
    });
  }
  ...
  function drawBuilding(ctx,b,w,h){
    ...
    g.addColorStop(0,"rgba(143,240,255,.20)");
    g.addColorStop(.58,"rgba(20,62,84,.42)");
    g.addColorStop(1,"rgba(1,7,15,.92)");
    ctx.fillStyle=g;
    ...
    ctx.strokeStyle="rgba(143,240,255,.30)";
    ctx.lineWidth=1.2;
  ...
  function drawSite(ctx,s,w,h,i){
    ...
    ctx.strokeStyle="rgba(143,240,255,.50)";
    ctx.lineWidth=1.2;
    ctx.stroke();
  ...
  function drawLinks(ctx,w,h){
    ...
    d.powers.forEach((p,idx)=>{
      ...
      ctx.strokeStyle=idx===0?"rgba(143,240,255,.82)":"rgba(251,191,36,.42)";
      ctx.beginPath();
      if(idx===0){
  ...
  function drawTeacherOverlay(ctx,w,h){
    ...
    round(ctx,18,18,250,148,18,true,true);
    ctx.font="bold 12px monospace";
    ctx.fillStyle="#42f56f";
    ctx.fillText("TEACHER VIEW · UE PROBE",36,42);
  ...
  function drawTopBadge(ctx,w,d){
    ctx.fillStyle="rgba(1,6,14,.82)";
    ctx.strokeStyle="rgba(66,245,111,.36)";
    round(ctx,w-260,18,236,78,20,true,true);
    ctx.fillStyle="#42f56f";
  ...
  function drawIQ(){
    const {ctx,w,h}=setupCanvas(iq,220);
    ...
    for(let n=0;n<900;n++){
      ...
      ctx.fillStyle=err?"rgba(255,65,99,.70)":"rgba(66,245,111,.62)";
  ...
  function drawPDP(){
    const {ctx,w,h}=setupCanvas(pdp,220);
  ...
  function drawH(){
    const {ctx,w,h}=setupCanvas(hCanvas,220);
    ...
    ctx.lineWidth=2;
    ctx.shadowColor="rgba(143,240,255,.60)";
    ctx.shadowBlur=9;
    ctx.stroke();
    ctx.shadowBlur=0;
  ...
  function drawBudget(){
    ...
    const rows=[
      ...
      ["Shadowing",`${state.shadow.toFixed(1)} dB`,"ostacoli / variabilità lenta"],
      ["Delay spread",`${d.ds.toFixed(0)} ns`,"dispersione temporale"],
      ["RSRP stimato",`${d.best.p.toFixed(1)} dBm`,"potenza ricevuta"],
      ["SINR stimato",`${d.sinr.toFixed(1)} dB`,"qualità/interferenza"]
    ];
  ...
  function setQualityClasses(d){
    const rsrpKpi = Array.from(document.querySelectorAll(".mini-kpi")).find(x => x.textContent.includes("RSRP"));
    const sinrKpi = Array.from(document.querySelectorAll(".mini-kpi")).find(x => x.textContent.includes("SINR"));
    const metrics = Array.from(document.querySelectorAll(".metric"));

    [...document.querySelectorAll(".mini-kpi,.metric")].forEach(el=>{
      el.classList.remove("critical","warning","good");
    });

    if(rsrpKpi){
      rsrpKpi.classList.add(d.best.p < -110 ? "critical" : d.best.p < -98 ? "warning" : "good");
    }
    if(sinrKpi){
      sinrKpi.classList.add(d.sinr < 3 ? "critical" : d.sinr < 10 ? "warning" : "good");
    }

    metrics.forEach(el=>{
      const t = el.textContent.toLowerCase();
      if(t.includes("evm")) el.classList.add(d.evm > 24 ? "critical" : d.evm > 12 ? "warning" : "good");
      if(t.includes("coverage")) el.classList.add(d.score < 35 ? "critical" : d.score < 60 ? "warning" : "good");
    });
  }
  ...
  function updateText(){
    ...
    $("truthScore").textContent=d.score.toFixed(0)+"%";
    setQualityClasses(d);

    let label="healthy";
    let note="Canale utilizzabile: il colore verde/ciano non è decorazione, ma sintesi di RSRP, SINR, EVM e dispersione. Qui l’allievo impara che la mappa è solo il primo indizio: la prova è negli strumenti.";
    if(d.score<35){
      label="critical";
      note="Qui nasce la lezione più importante: quando compare rosso, deve esserci una ragione fisica. NLOS, SINR basso, EVM alta o delay spread spiegano il degrado. Nulla è casuale.";
    }else if(d.evm>14 || d.ds>300){
      label="dispersive";
      note="Il campo non è necessariamente perso: è dispersivo. Il PDP mostra multipath significativo e la costellazione si apre. Caso eccellente per spiegare ISI, selettività in frequenza, OFDM e cyclic prefix.";
    }else if(d.sinr<8){
      label="interference";
      note="RSRP accettabile ma SINR basso: il problema non è copertura pura, è interferenza o sovrapposizione di celle. Qui si insegna best-server, handover margin e dominio interferenziale.";
    }
    $("truthLabel").textContent=label;
...
function installV84CR2(){
  document.body.classList.add("v84c-r2");
  document.body.classList.add("v84d-truth");

  const heroTitle = document.querySelector("h1");
  if(heroTitle) heroTitle.textContent = "RF Sapienza Truth Palette";

  const eyebrow = document.querySelector(".eyebrow");
  if(eyebrow) eyebrow.textContent = "TRFMC v0.84D · RF TRUTH PALETTE + MASTERCLASS INSPECTOR";

  const stageTitle = document.querySelector(".stage-title h2");
  if(stageTitle) stageTitle.textContent = "Calibrated RF Field";


  if(!document.querySelector(".v84d-inspector")){
    const instruments = document.querySelector(".instruments");
    const inspector = document.createElement("section");
    inspector.className = "v84d-inspector";
    inspector.innerHTML = `
      <article>
        <h2>Truth Palette</h2>
        <p>La scala colore è stata corretta: rosso non significa potenza o spettacolo, ma degrado operativo. Verde/ciano significa servizio utile, ambra significa margine, blu significa debolezza/NLOS.</p>
      ...
      <article>
        <h2>Regola del Maestro</h2>
        <p>Non si insegna una heatmap: si insegna una diagnosi. Prima si osserva la geometria, poi si controllano RSRP e SINR, poi si verifica PDP/IQ/EVM.</p>
        <code>Mappa = ipotesi · Strumento = prova · Metodo = sapienza</code>
      </article>
      <article>
        <h2>Diagnosi Canale</h2>
        <p>Delay spread alto e K-factor basso allargano la costellazione, rendono il canale selettivo e abbassano la qualità anche quando il campo sembra ancora presente.</p>
        <code>Qualità reale = RSRP + SINR + PDP + EVM + coerenza</code>
      </article>
    `;
    instruments?.insertAdjacentElement("beforebegin", inspector);
  }


  if(!document.querySelector(".v84c-masterclass")){
    const doctrine = document.querySelector(".doctrine");
    const master = document.createElement("section");
    master.className = "v84c-masterclass";
    master.innerHTML = `
      <article>
        <h2>Metodo</h2>
        <p>La pagina deve insegnare una catena causale: geometria, attenuazione, multipath, risposta in frequenza, IQ, EVM e decisione radio.</p>
        <code>Osservo → misuro → confronto → interpreto → insegno</code>
      </article>
      <article>
        <h2>Verità RF</h2>
        <p>La potenza ricevuta non è qualità. Un RSRP accettabile può convivere con SINR basso, delay spread elevato e costellazione degradata.</p>
        <code>Qualità = funzione(RSRP, SINR, PDP, EVM, coerenza)</code>
      </article>
      <article>
        <h2>Uso didattico</h2>
        <p>L’allievo deve leggere lo strumento come un tecnico: non fidarsi del colore, ma cercare coerenza tra campo, canale e misura.</p>
        <code>La mappa suggerisce; lo strumento dimostra.</code>
      </article>
    `;
    doctrine?.insertAdjacentElement("beforebegin", master);
  }
}
