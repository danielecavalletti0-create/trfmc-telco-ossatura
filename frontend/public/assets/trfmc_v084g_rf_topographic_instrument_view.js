(function(){
  ...
  document.addEventListener("DOMContentLoaded",()=>{ document.body.classList.add("v84f-dark-field"); document.body.classList.add("v84g-topographic"); });
  ...
  function bind(){
    ...
    $("btnBalanced").onclick=()=>setScenario({freq:3500,eirp:46,pathN:3.15,shadow:7,delay:115,kfactor:12,beam:18});
  ...
  function channelAt(x,y){
    ...
    const score=Math.max(0,Math.min(100,52+(best.p+95)*1.06+sinr*2.85-evm*.94-ds/52));
    return {powers,best,second,sinr,gap,ds,evm,coh,score};
  }

  function classOf(c){
    if(c.best.p < -118 || c.sinr < -4 || c.evm > 35 || c.score < 8) return "critical";
    if(c.best.p < -110 || c.sinr < 3 || c.score < 30) return "weak";
    if(c.best.p < -98 || c.sinr < 9 || c.score < 54) return "edge";
  ...
  function colorFor(cls){
    /*
      v84G topographic rule:
      - no class may become a curtain
      - critical is dark blue/violet surface, red remains contour only
      - weak is blue, edge is amber, good is green, excellent is cyan
    */
    if(cls==="critical") return [28,38,72,112];       // critical area = dark blue-violet, not black/red
    if(cls==="weak")     return [24,92,148,124];      // weak/NLOS = blue
    if(cls==="edge")     return [156,126,46,116];     // margin = muted amber
    if(cls==="excellent")return [142,230,214,122];    // excellent = cyan-green
    return [44,182,122,120];                          // good = controlled green
  }
  ...
  function drawField(){
    ...
    ctx.filter="blur(1.45px) saturate(.76) brightness(.88) contrast(1.10)";
  ...
  function drawContourLabels(ctx,w,h){
    const labels = [
      {x:.11,y:.18,t:"weak/NLOS contour",c:"rgba(46,167,255,.72)"},
      {x:.46,y:.23,t:"usable service ridge",c:"rgba(66,245,111,.72)"},
      {x:.70,y:.68,t:"cell-edge margin",c:"rgba(251,191,36,.70)"},
      {x:.83,y:.36,t:"critical contour only",c:"rgba(255,59,92,.70)"}
    ];
    ctx.save();
  ...
  function updateText(){
    ...
    if(d.score<35){
      label="critical";
      note="Vista topografica: niente tende rosse e niente tende nere. La superficie resta continua; le criticità diventano contorni locali, mentre la prova tecnica rimane in IQ, PDP, H(f) e link budget.";
