(function(){
  ...
  function bind(){
    ...
    $("btnBalanced").onclick=()=>{
      setScenario({freq:3500,eirp:46,pathN:3.1,shadow:10,delay:110,kfactor:11,beam:18});
    };
  ...
  function channelAt(x,y){
    ...
    const score=Math.max(0,Math.min(100,48+(best.p+95)*1.35+sinr*2.8-evm*.90-ds/48));
    return {powers,best,sinr,ds,evm,coh,score};
  }

  function palette(score,rsrp){
    /*
      v84C dark calibrated RF scale:
      - no white wash
      - green means stable service
      - amber means margin/cell-edge
      - red only severe local hot/degraded concentration
      - black/blue means outage or weak/NLOS
    */
    if(score<18 || rsrp<-112) return [2,7,16,238];
    if(score<34) return [13,48,92,224];
    if(score<52) return [18,105,156,214];
    if(score<70) return [28,178,132,205];
    if(score<86) return [94,212,89,198];
    if(score<94) return [220,184,56,190];
    return [232,80,72,176];
  }
  ...
  function drawField(){
    ...
    for(let yy=0;yy<oh;yy++){
      for(let xx=0;xx<ow;xx++){
        ...
        const col=palette(Math.min(96,c.score),c.best.p);
    ...
    ctx.filter="blur(3px) saturate(.95) brightness(.88)";
  ...
  function updateText(){
    ...
    let label="healthy";
    let note="Canale utilizzabile e didatticamente leggibile: il campo RF, la costellazione, il PDP e la risposta in frequenza sono coerenti. Qui si insegna il metodo: non guardare solo il colore, ma correlare potenza, qualità e dispersione.";
    if(d.score<35){
      label="critical";
      note="Qui nasce la lezione più importante: la potenza ricevuta può ingannare. NLOS, delay spread e interferenza degradano la qualità. L’allievo deve imparare a leggere IQ, PDP e SINR prima di fidarsi della mappa.";
...
function installV84CR2(){
  ...
  if(heroTitle) heroTitle.textContent = "RF Sapienza Console R2";

  const eyebrow = document.querySelector(".eyebrow");
  if(eyebrow) eyebrow.textContent = "TRFMC v0.84C · RF ENGINEERING MASTERCLASS R2";
