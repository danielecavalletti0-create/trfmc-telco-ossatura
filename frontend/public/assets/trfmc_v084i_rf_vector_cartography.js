(function(){
  ...
  document.addEventListener("DOMContentLoaded",()=>{ document.body.classList.add("v84f-dark-field"); document.body.classList.add("v84g-topographic"); document.body.classList.add("v84h-atlas"); document.body.classList.add("v84i-vector"); });
  ...
  function bind(){
    ...
    $("btnBalanced").onclick=()=>setScenario({freq:3500,eirp:46,pathN:3.0,shadow:5,delay:95,kfactor:14,beam:18});
  ...
  function channelAt(x,y){
    ...
    const score=Math.max(0,Math.min(100,56+(best.p+95)*.98+sinr*2.68-evm*.84-ds/62));
    return {powers,best,second,sinr,gap,ds,evm,coh,score};
  }

  function classOf(c){
    if(c.best.p < -122 || c.sinr < -7 || c.evm > 40 || c.score < 5) return "critical";
    if(c.best.p < -113 || c.sinr < 1.5 || c.score < 26) return "weak";
    if(c.best.p < -100 || c.sinr < 8 || c.score < 52) return "edge";
  ...
  function colorFor(c,cls){
    /*
      v84I vector cartography:
      field fill is deliberately subordinate.
      engineering truth is carried by vectors, isolines and instruments.
    */
    const q = Math.max(0, Math.min(1, c.score / 100));
    let r,g,b,a;

    if(q < .22){
      r = 18; g = 62; b = 100; a = 82;
    }else if(q < .42){
      r = 20; g = 106; b = 142; a = 88;
    }else if(q < .62){
      r = 38; g = 146; b = 124; a = 94;
    }else if(q < .80){
      r = 50; g = 180; b = 112; a = 96;
    }else{
      r = 138; g = 230; b = 208; a = 92;
    }

    if(cls === "edge"){
      r = Math.round(r * .86 + 180 * .14);
      g = Math.round(g * .86 + 138 * .14);
      b = Math.round(b * .86 + 46  * .14);
    }

    if(cls === "critical"){
      r = Math.round(r * .86 + 92 * .14);
      g = Math.round(g * .86 + 32 * .14);
      b = Math.round(b * .86 + 70 * .14);
      a = 78;
    }
  ...
  function drawField(){
    ...
    ctx.filter="blur(.75px) saturate(.88) brightness(1.00) contrast(1.15)";
    ctx.drawImage(off,0,0,w,h);
    ctx.restore();

    drawGrid(ctx,w,h);
    drawDiagnosticContours(ctx,w,h,data,cols,rows);
    drawContourLabels(ctx,w,h);
    drawAtlasScale(ctx,w,h);
    drawVectorCartography(ctx,w,h);
    drawBeams(ctx,w,h);
  ...
  function drawDiagnosticContours(ctx,w,h,data,cols,rows){
    ...
    classes.forEach(cls=>{
      ...
      ctx.lineWidth=cls==="critical"?1.25:1.05;
      ctx.setLineDash(cls==="critical"?[3,8]:cls==="edge"?[9,7]:[]);
  ...
  function drawContourLabels(ctx,w,h){
    const labels = [
      {x:.11,y:.18,t:"NLOS corridor",c:"rgba(46,167,255,.72)"},
      {x:.46,y:.23,t:"serving dominance",c:"rgba(66,245,111,.72)"},
      {x:.70,y:.68,t:"cell-edge isoline",c:"rgba(251,191,36,.70)"},
      {x:.83,y:.36,t:"SINR stress isoline",c:"rgba(255,59,92,.70)"}
    ];
    ctx.save();
    ...
    labels.forEach(l=>{
      ...
      ctx.strokeStyle="rgba(143,240,255,.20)";
      round(ctx,x-8,y-15,Math.min(190,l.t.length*7.3+18),24,10,true,true);
      ctx.fillStyle=l.c;
  ...
  function drawAtlasScale(ctx,w,h){
    const x = 18;
    const y = h - 112;
    const ww = 310;
    const hh = 74;
    ...
    round(ctx,x,y,ww,hh,16,true,true);

    ctx.fillStyle="#8ff0ff";
    ctx.font="bold 11px monospace";
    ctx.fillText("RF ATLAS SCALE · NOT DECORATIVE",x+14,y+22);
    ...
    stops.forEach((s,i)=>{
      const sx=x+14+i*72;
      ctx.fillStyle=s[1];
      ctx.fillRect(sx,y+38,44,6);
      ctx.fillStyle="rgba(232,250,255,.62)";
      ctx.font="9px monospace";
      ctx.fillText(s[0],sx,y+58);
    });
  ...
  function drawVectorCartography(ctx,w,h){
    const d = channelAt(ue.x,ue.y);
    const ux = ue.x*w, uy = ue.y*h;
    ...
    ctx.lineWidth=1;
    for(let r=90;r<Math.max(w,h);r+=140){
      ctx.beginPath();
      ctx.arc(ux,uy,r,0,Math.PI*2);
      ctx.stroke();
    }

    // serving and neighbor vectors
    d.powers.forEach((p,idx)=>{
      const sx=p.site.x*w, sy=p.site.y*h;
      const isBest = idx===0;
      ctx.setLineDash(isBest?[]:[8,7]);
      ctx.lineWidth=isBest?2.3:1.1;
      ctx.strokeStyle=isBest?"rgba(143,240,255,.92)":"rgba(251,191,36,.46)";
      ctx.beginPath();
      ctx.moveTo(sx,sy);
      ctx.lineTo(ux,uy);
      ctx.stroke();
      ...
      // arrow head
      const a=Math.atan2(uy-sy,ux-sx);
      const ax=ux-Math.cos(a)*22;
      const ay=uy-Math.sin(a)*22;
      ctx.beginPath();
      ctx.moveTo(ax,ay);
      ctx.lineTo(ax-Math.cos(a-.42)*13,ay-Math.sin(a-.42)*13);
      ctx.moveTo(ax,ay);
      ctx.lineTo(ax-Math.cos(a+.42)*13,ay-Math.sin(a+.42)*13);
      ctx.stroke();

      // vector label
      const mx=sx+(ux-sx)*.50;
      const my=sy+(uy-sy)*.50;
      ctx.fillStyle="rgba(1,6,14,.82)";
      ctx.strokeStyle=isBest?"rgba(143,240,255,.34)":"rgba(251,191,36,.22)";
      round(ctx,mx-48,my-14,96,24,10,true,true);
      ctx.fillStyle=isBest?"#8ff0ff":"#fbbf24";
      ctx.font="bold 9px monospace";
      ctx.textAlign="center";
      ctx.fillText(isBest?"SERVING VECTOR":"NEIGHBOR",mx,my+3);
      ctx.textAlign="left";
    });
    ...
    // UE measurement ring
    const quality = Math.max(0,Math.min(1,d.score/100));
    ctx.strokeStyle=quality>.65?"rgba(66,245,111,.82)":quality>.40?"rgba(251,191,36,.78)":"rgba(255,59,92,.70)";
    ctx.lineWidth=2;
    ctx.beginPath();
    ctx.arc(ux,uy,38+quality*18,0,Math.PI*2);
    ctx.stroke();
  ...
  function drawBuilding(ctx,b,w,h){
    ...
    ctx.strokeStyle="rgba(143,240,255,.62)";
    ctx.lineWidth=1.45;
  ...
  function updateText(){
    ...
    if(d.score<35){
      label="critical";
      note="Vista Vector Cartography: il colore è solo una base. La diagnosi passa dai vettori di serving, dalle isolinee, dagli ostacoli, dal probe UE e dalla coerenza fra IQ, PDP, H(f) e link budget.";
