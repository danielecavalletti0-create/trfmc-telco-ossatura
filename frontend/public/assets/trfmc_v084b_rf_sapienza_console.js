(function(){
  ...
  function bind(){
    ...
    $("btnTeach").onclick=()=>{
      ...
      $("btnTeach").textContent=state.teacher ? "Teacher view ON" : "Teacher view";
      updateAll();
    };
  ...
  function channelAt(x,y){
    ...
    const score=Math.max(0,Math.min(100,55+(best.p+95)*1.65+sinr*2.4-evm*.75-ds/55));
    return {powers,best,sinr,ds,evm,coh,score};
  }

  function palette(score,rsrp){
    if(score<18 || rsrp<-112) return [3,8,15,236];
    if(score<34) return [21,68,118,220];
    if(score<52) return [34,142,210,210];
    if(score<70) return [51,210,142,205];
    if(score<86) return [100,236,105,205];
    return [196,255,222,220];
  }
  ...
  function drawField(){
    ...
    bg.addColorStop(.55,"#031022");
    bg.addColorStop(1,"#01050d");
    ctx.fillStyle=bg;
    ...
    for(let yy=0;yy<oh;yy++){
      for(let xx=0;xx<ow;xx++){
        ...
        const col=palette(c.score,c.best.p);
  ...
  function updateText(){
    ...
    let label="healthy";
    let note="Il canale è coerente: RSRP, SINR, EVM e PDP raccontano la stessa storia. Questa è la condizione ideale per spiegare agli allievi la differenza tra potenza e qualità.";
    if(d.score<35){
      label="critical";
      note="Qui nasce la lezione vera: la potenza ricevuta non basta. Il canale è degradato da NLOS, delay spread e interferenza. L’allievo deve guardare IQ e PDP prima della mappa.";
    }else if(d.evm>14 || d.ds>300){
      label="dispersive";
      note="Il campo non è perso, ma è dispersivo. Il PDP mostra multipath significativo e la costellazione si allarga. È un caso perfetto per spiegare ISI, OFDM e cyclic prefix.";
    }else if(d.sinr<8){
      label="interference";
