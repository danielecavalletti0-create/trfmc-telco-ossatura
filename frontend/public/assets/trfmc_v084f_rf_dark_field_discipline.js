(function(){
  ...
  document.addEventListener("DOMContentLoaded",()=>document.body.classList.add("v84f-dark-field"));
  ...
  function bind(){
    ...
    $("btnBalanced").onclick=()=>setScenario({freq:3500,eirp:46,pathN:3.2,shadow:7,delay:120,kfactor:11,beam:18});
  ...
  function channelAt(x,y){
    ...
    const score=Math.max(0,Math.min(100,50+(best.p+95)*1.10+sinr*3.00-evm*1.00-ds/48));
    return {powers,best,second,sinr,gap,ds,evm,coh,score};
  }

  function classOf(c){
    if(c.best.p < -116 || c.sinr < -2 || c.evm > 32 || c.score < 12) return "critical";
    if(c.best.p < -108 || c.sinr < 4 || c.score < 32) return "weak";
  ...
  function colorFor(cls){
    /*
      v84F dark-field rule:
      - critical is NOT a filled red curtain
      - critical surface is dark neutral, red appears only in contours/labels
      - the field stays subordinate to instruments
    */
    if(cls==="critical") return [18,10,18,92];        // critical = dark maroon/black, not red fill
    if(cls==="weak")     return [18,74,126,118];      // weak/NLOS = restrained blue
    if(cls==="edge")     return [138,112,42,108];     // margin = muted amber
    if(cls==="excellent")return [128,218,202,110];    // excellent = pale cyan/green
    return [38,166,112,108];                          // good = controlled green
  }
  ...
  function drawField(){
    ...
    ctx.filter="blur(1.7px) saturate(.62) brightness(.74) contrast(1.14)";
  ...
  function drawDiagnosticContours(ctx,w,h,data,cols,rows){
    ...
    classes.forEach(cls=>{
      ...
      ctx.lineWidth=cls==="critical"?1.35:1.05;
      ctx.setLineDash(cls==="critical"?[4,7]:[]);
  ...
  function updateText(){
    ...
    if(d.score<35){
      label="critical";
      note="Degrado reale: in v84F il rosso non riempie più il campo. Compare come contorno locale quando SINR, RSRP, EVM o delay spread lo giustificano. La prova resta negli strumenti, non nella macchia.";
