(function(){
  ...
  document.addEventListener("DOMContentLoaded",()=>{ document.body.classList.add("v84f-dark-field"); document.body.classList.add("v84g-topographic"); document.body.classList.add("v84h-atlas"); });
  ...
  function bind(){
    ...
    $("btnBalanced").onclick=()=>setScenario({freq:3500,eirp:46,pathN:3.05,shadow:6,delay:105,kfactor:13,beam:18});
  ...
  function channelAt(x,y){
    ...
    const score=Math.max(0,Math.min(100,54+(best.p+95)*1.02+sinr*2.75-evm*.88-ds/58));
    return {powers,best,second,sinr,gap,ds,evm,coh,score};
  }

  function classOf(c){
    if(c.best.p < -120 || c.sinr < -6 || c.evm > 38 || c.score < 6) return "critical";
    if(c.best.p < -112 || c.sinr < 2 || c.score < 28) return "weak";
  ...
  function colorFor(c,cls){
    /*
      v84H atlas rule:
      the fill is continuous and never becomes a curtain.
      classes are used for isolines; the surface uses a soft scalar field.
    */
    const q = Math.max(0, Math.min(1, c.score / 100));
    const stress = Math.max(0, Math.min(1, (18 - c.sinr) / 24 + (c.evm - 10) / 40));
    let r,g,b,a;

    if(q < .25){
      r = 18; g = 54; b = 92; a = 106;       // weak field, not black
    }else if(q < .45){
      r = 20; g = 104; b = 146; a = 112;     // blue/teal NLOS
    }else if(q < .62){
      r = 42; g = 150; b = 124; a = 118;     // usable teal
    }else if(q < .78){
      r = 62; g = 182; b = 112; a = 118;     // good green
    }else{
      r = 130; g = 222; b = 196; a = 116;    // excellent cyan-green
    }

    if(cls === "edge"){
      r = Math.round(r * .80 + 180 * .20);
      g = Math.round(g * .82 + 138 * .18);
      b = Math.round(b * .82 + 46  * .18);
    }

    if(cls === "critical"){
      r = Math.round(r * .76 + 78 * .24);
      g = Math.round(g * .76 + 42 * .24);
      b = Math.round(b * .76 + 92 * .24);
      a = 100;
    }

    a = Math.round(a * (1 - Math.min(.32, stress * .20)));
    return [r,g,b,a];
  }
  ...
  function drawField(){
    ...
    ctx.filter="blur(1.15px) saturate(.82) brightness(.94) contrast(1.12)";
  ...
  function drawContourLabels(ctx,w,h){
    const labels = [
      ...
      {x:.46,y:.23,t:"serving ridge",c:"rgba(66,245,111,.72)"},
      {x:.70,y:.68,t:"cell-edge isoline",c:"rgba(251,191,36,.70)"},
      {x:.83,y:.36,t:"SINR stress",c:"rgba(255,59,92,.70)"}
    ];
    ctx.save();
  ...
  function drawBuilding(ctx,b,w,h){
    ...
    ctx.strokeStyle="rgba(143,240,255,.46)";
    ctx.lineWidth=1.2;
  ...
  function updateText(){
    ...
    if(d.score<35){
      label="critical";
      note="Vista RF Atlas: il campo è continuo e leggibile, le isolinee indicano soglie operative, gli oggetti restano riconoscibili. La diagnosi nasce dalla correlazione tra superficie RF, IQ, PDP, risposta in frequenza e link budget.";
