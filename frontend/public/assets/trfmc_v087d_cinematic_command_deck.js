(function(){
  const ROOT = document.querySelector('[data-trfmc-page="cinematic-command-deck-v87d"]');
  if(!ROOT) return;
...
state: ${state.ok[r.file] ? "reachable" : "not validated"}</code>
  ...
  async function boot(){
    ...
    try{
      const res = await fetch("/reports/trfmc_v87d_cinematic_command_deck_manifest.json?ts=" + Date.now(), {cache:"no-store"});
