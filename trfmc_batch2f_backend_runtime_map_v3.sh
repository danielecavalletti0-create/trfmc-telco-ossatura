...
OUT="$BASE/runtime/quality/TRFMC_BATCH2F_BACKEND_RUNTIME_MAP_V3_$TS"
...
echo "============================================================"
echo "TRFMC_BATCH2F_BACKEND_RUNTIME_MAP_V3"
...
if [ -n "${PARENT_PID:-}" ] && [ -r "/proc/$PARENT_PID/cmdline" ]; then
  PARENT_CMDLINE="$(tr '\0' ' ' < "/proc/$PARENT_PID/cmdline" 2>/dev/null | sed 's/[[:space:]]*$//' || true)"
fi

{
  echo -e "field\tvalue"
  echo -e "pid\t$PID"
  echo -e "parent_pid\t${PARENT_PID:-UNKNOWN}"
...
if [ -n "${PARENT_PID:-}" ]; then
  ps -fp "$PARENT_PID" | tee "$OUT/ps_parent_pid.txt" || true
else
  echo "NO_PARENT_PID" | tee "$OUT/ps_parent_pid.txt"
fi
...

{
  ...
  if [ -f backend/routers/rf_normalized_trace_v1.py ]; then
    grep -n 'prefix="/api/rfpro/normalized"\|@router.get("/spectrum/trace")\|def normalized_spectrum_trace' \
      ...
      | awk -F: 'BEGIN{OFS="\t"} {print "backend/routers/rf_normalized_trace_v1.py","router_source","PASS",$0}' \
      || true
  else
  ...
  if [ -f backend/main_v580.py ]; then
    grep -n 'TRFMC BATCH2F RF NORMALIZED TRACE ROUTER START\|rf_normalized_trace_v1\|include_router' \
      ...
      | awk -F: 'BEGIN{OFS="\t"} {print "backend/main_v580.py","include_marker","PASS",$0}' \
      || true
  else
...
{
  "timestamp": "$TS",
  "operation": "TRFMC_BATCH2F_BACKEND_RUNTIME_MAP_V3",
  "mutation": false,
  "pid": "$PID",
  "parent_pid": "${PARENT_PID:-UNKNOWN}",
  "cwd": "$CWD",
  "cmdline": "$CMDLINE",
...
ln -sfn "$OUT" "$BASE/runtime/quality/latest_batch2f_backend_runtime_map_v3"
...
echo
echo "============================================================"
echo "TRFMC_BATCH2F_BACKEND_RUNTIME_MAP_V3 COMPLETATO"
echo "Output: $OUT"
echo "============================================================"
