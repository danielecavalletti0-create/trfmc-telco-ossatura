...
WEBGL="$PUBLIC/trfmc_3d_rf_asset_renderer_webgl_v2r2_reality.html"
WEBGL_V2R1="$PUBLIC/trfmc_3d_rf_asset_renderer_webgl_v2r1_detail_boost.html"
...
OUT="$BASE/runtime/quality/TRFMC_PROMOTE_WEBGL_V2R2_REALITY_INTO_V6R3_$TS"
BK="$BASE/runtime/backups/PROMOTE_WEBGL_V2R2_REALITY_INTO_V6R3_$TS"

mkdir -p "$OUT" "$BK" "$BASE/runtime/quality" "$BASE/runtime/freezes"

echo "============================================================"
echo "TRFMC - PROMOTE WEBGL V2R2 REALITY INTO V6R3"
echo "official 3D RF visual twin leaf module"
echo "============================================================"
...
ls -lh "$WEBGL" || {
  echo "ERRORE: V2R2 Reality non trovata."
  echo "Devi prima creare:"
  echo "$WEBGL"
  exit 1
}
...
echo
echo "[3/8] Gate preliminare WebGL V2R2"
cat > "$OUT/http_pre.tsv" <<'EOFHTTP'
url	status	bytes
EOFHTTP
...
for u in \
  ...
  /api/health
do
  http_probe "$u" >> "$OUT/http_pre.tsv"
done

grep -nEi '<iframe|src="/trfmc_(supervisor|unified|official_safe_entrypoint)' \
  "$WEBGL" > "$OUT/webgl_iframe_refs_pre.txt" 2>/dev/null || true

grep -nEi 'http://|https://|cdn\.|unpkg|jsdelivr|cdnjs' \
  "$WEBGL" > "$OUT/webgl_external_refs_pre.txt" 2>/dev/null || true

grep -nEi 'ACESFilmicToneMapping|SRGBColorSpace|PCFSoftShadowMap|radiation pattern|primary/secondary|RF air particles|proceduralNoiseTexture|brushedMetalTexture|Box3|WebGLRenderer|RoundedBoxGeometry|OrbitControls' \
  "$WEBGL" > "$OUT/webgl_reality_markers_pre.txt" 2>/dev/null || true

echo
echo "[4/8] Patch V6R3: WebGL V2R2 diventa modulo 3D ufficiale"
python3 - <<'PY'
...
new_url = "/trfmc_3d_rf_asset_renderer_webgl_v2r2_reality.html"
...
for u in old_urls:
    if u in s:
        s = s.replace(u, new_url)
        changed = True

module = ''' webgl3d:{icon:"3D",name:"3D RF Visual Twin WebGL",desc:"PBR · shadows · radiation lobes · antenna/AAU/RRU/filter/dish meshes · air RF layer",url:"/trfmc_3d_rf_asset_renderer_webgl_v2r2_reality.html"},
'''

quick_button = '''        <button onclick="load('webgl3d')">3D WebGL</button>
'''
...
if new_url not in s:
    ...
    # Se URL presente ma manca la chiave webgl3d, inserisco comunque il modulo.
    if "webgl3d" not in s:
        if "const M={\n" in s:
            s = s.replace("const M={\n", "const M={\n" + module, 1)
            changed = True

if "load('webgl3d')" not in s:
    markers = [
        '''        <button onclick="load('metrology')">RF Metrology</button>''',
        '''        <button onclick="load('fftgapless')">FFT Gapless</button>''',
    ...
    for marker in markers:
        ...
        print("WARN: marker pulsante non trovato; modulo inserito nel registry")

# Aggiorno eventuale descrizione header.
s = s.replace(
    "RF/Telco/Cyber command console · Signal Intelligence + FFT Gapless + RF Metrology leaf modules · no shell nesting",
    "RF/Telco/Cyber command console · Signal Intelligence + FFT Gapless + RF Metrology + 3D WebGL Visual Twin leaf modules · no shell nesting"
)
...
# Se esiste un vecchio nome modulo 3D generico lo lasciamo, ma lo puntiamo alla V2R2.
s = re.sub(
    r'([A-Za-z0-9_]+:\{icon:"(?:3D|XR|CAD)".*?url:")/trfmc_3d_rf_asset_renderer_[^"]+(")',
    r'\1/trfmc_3d_rf_asset_renderer_webgl_v2r2_reality.html\2',
    s,
    flags=re.S
)
...
if changed:
    print("PATCH_OK: WebGL V2R2 Reality agganciata in V6R3")
else:
    print("NO_CHANGE: WebGL V2R2 era già agganciata")
PY
...
grep -nEi '<iframe[^>]+src="/trfmc_(supervisor|unified|official_safe_entrypoint)' \
  "$V6R3" "$WEBGL" "$SIG" "$FFT" "$MET" > "$OUT/nested_shell_iframe_refs.txt" 2>/dev/null || true

grep -nEi '<iframe' \
  "$WEBGL" > "$OUT/webgl_iframe_refs_post.txt" 2>/dev/null || true

grep -nEi 'http://|https://|cdn\.|unpkg|jsdelivr|cdnjs' \
  "$V6R3" "$WEBGL" "$SIG" "$FFT" "$MET" > "$OUT/external_refs.txt" 2>/dev/null || true

grep -nEi 'ACESFilmicToneMapping|SRGBColorSpace|PCFSoftShadowMap|radiation pattern|primary/secondary|RF air particles|proceduralNoiseTexture|brushedMetalTexture|Box3|WebGLRenderer|RoundedBoxGeometry|OrbitControls' \
  "$WEBGL" > "$OUT/reality_markers.txt" 2>/dev/null || true

{
  ...
  grep -n "/trfmc_3d_rf_asset_renderer_webgl_v2r2_reality.html" "$V6R3" || true
  grep -n "load('sigintel')" "$V6R3" || true
  grep -n "load('fftgapless')" "$V6R3" || true
  grep -n "load('metrology')" "$V6R3" || true
  grep -n "load('webgl3d')" "$V6R3" || true
...
webgl_iframes = sum(1 for x in (out / "webgl_iframe_refs_post.txt").read_text(errors="ignore").splitlines() if x.strip())
...
has_webgl = "webgl3d" in integration and "/trfmc_3d_rf_asset_renderer_webgl_v2r2_reality.html" in integration
...
data = {
    ...
    "webgl_v2r2_reality_leaf": "http://127.0.0.1:5173/trfmc_3d_rf_asset_renderer_webgl_v2r2_reality.html",
    ...
    "webgl_iframe_refs": webgl_iframes,
    ...
    "v6r3_has_webgl3d_module": has_webgl,
    "result": result
}
...
ln -sfn "$(basename "$OUT")" "$BASE/runtime/quality/latest_promote_webgl_v2r2_reality_into_v6r3"
...
echo
echo "=== WEBGL IFRAME ==="
cat "$OUT/webgl_iframe_refs_post.txt"
...
if grep -q '^PASS$' "$OUT/result.flag"; then
  FREEZE="$BASE/runtime/freezes/TRFMC_PORTAL_PASS_WEBGL_V2R2_REALITY_AS_V6R3_LEAF_$TS.tar.gz"
  ...
  echo
  echo "============================================================"
  echo "PROMOZIONE VALIDATA: WEBGL V2R2 REALITY È MODULO FOGLIA UFFICIALE IN V6R3"
  echo
  echo "Apri V6R3:"
  echo "http://127.0.0.1:5173/trfmc_official_safe_entrypoint_v6r3_command_center.html"
  echo
  echo "Modulo diretto 3D WebGL Reality:"
