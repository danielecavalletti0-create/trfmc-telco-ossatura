...
OUT="$BASE/runtime/quality/TRFMC_RESCUE_RF_SIGNAL_PROMOTION_V1_REPORT_$TS"
...
echo "============================================================"
echo "TRFMC_RESCUE_RF_SIGNAL_PROMOTION_V1_REPORT"
...

{
  ...
  if grep -q "RFSignalAnalyzerPromotionV1" "$EXP" 2>/dev/null; then
    echo -e "EngineeringConsoleExpansionV4 imports_or_mounts_RFSignalAnalyzerPromotionV1\tPASS\t$(grep -n "RFSignalAnalyzerPromotionV1" "$EXP" | tr '\n' ' ' | sed 's/[[:space:]]*$//')"
  else
    echo -e "EngineeringConsoleExpansionV4 imports_or_mounts_RFSignalAnalyzerPromotionV1\tFAIL\t-"
  fi

  if grep -q "data-trfmc-rf-signal-promotion-v1" "$COMP" 2>/dev/null; then
    echo -e "RFSignalAnalyzerPromotionV1_has_DOM_marker\tPASS\t$(grep -n "data-trfmc-rf-signal-promotion-v1" "$COMP" | tr '\n' ' ' | sed 's/[[:space:]]*$//')"
  else
    echo -e "RFSignalAnalyzerPromotionV1_has_DOM_marker\tFAIL\t-"
  fi

  if grep -q "RFSignalAnalyzerWorkbenchV3" "$COMP" 2>/dev/null; then
    echo -e "RFSignalAnalyzerWorkbenchV3_mounted\tPASS\t$(grep -n "RFSignalAnalyzerWorkbenchV3" "$COMP" | tr '\n' ' ' | sed 's/[[:space:]]*$//')"
  else
    echo -e "RFSignalAnalyzerWorkbenchV3_mounted\tFAIL\t-"
  fi

  if grep -q "RFInstrumentDockV4" "$COMP" 2>/dev/null; then
    echo -e "RFInstrumentDockV4_mounted\tPASS\t$(grep -n "RFInstrumentDockV4" "$COMP" | tr '\n' ' ' | sed 's/[[:space:]]*$//')"
  else
    echo -e "RFInstrumentDockV4_mounted\tFAIL\t-"
  fi

  if grep -q "TrueSpectrumAnalyzer" "$COMP" 2>/dev/null; then
    echo -e "TrueSpectrumAnalyzer_mounted\tPASS\t$(grep -n "TrueSpectrumAnalyzer" "$COMP" | tr '\n' ' ' | sed 's/[[:space:]]*$//')"
  else
    echo -e "TrueSpectrumAnalyzer_mounted\tFAIL\t-"
  fi

  IFRAME_COUNT="$(grep -RIn "<iframe" "$COMP" "$EXP" 2>/dev/null | wc -l | tr -d ' ')"
  ...
  PUBLIC_PATCH_COUNT="$(grep -RIn "frontend/public\|/assets/trfmc_.*v51\|trfmc_.*v51r" "$COMP" "$EXP" "$CSS" 2>/dev/null | wc -l | tr -d ' ')"
...
DOM_TEXT_COUNT="$(grep -c "RF Signal Analyzer: teoria, DSP, visual asset, contract, scenario, QA" "$DOM" 2>/dev/null || true)"
...
## Diagnosi
La UI mostra che il modulo RF/Signal è stato innestato, ma il precedente script non ha prodotto il report/symlink atteso.

## Esito atteso del rescue
- Verificare esistenza dei file.
- Verificare import/mount.
- Verificare assenza iframe.
- Verificare assenza patch runtime/public V51.
- Verificare build.
- Verificare HTTP.
- Verificare DOM marker.
- Produrre summary e symlink stabile.

## Nota
Se \`/api/rfpro/spectrum/sweep\` risulta NON_200 ma il componente resta montato e build/DOM sono OK, non è un errore del frontend: è un contratto endpoint da completare nel batch API successivo.
MD
...
{
  "timestamp": "$TS",
  "operation": "TRFMC_RESCUE_RF_SIGNAL_PROMOTION_V1_REPORT",
  "mutation": false,
...
echo
echo "============================================================"
echo "TRFMC_RESCUE_RF_SIGNAL_PROMOTION_V1_REPORT COMPLETATO"
echo "Output: $OUT"
echo "============================================================"
