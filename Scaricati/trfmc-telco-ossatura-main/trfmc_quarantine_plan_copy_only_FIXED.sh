...
OUT="$BASE/runtime/quality/TRFMC_QUARANTINE_PLAN_COPY_ONLY_FIXED_$TS"
Q="$BASE/_archive/quarantine/TRFMC_QUARANTINE_COPY_ONLY_FIXED_$TS"
...
echo "============================================================"
echo "TRFMC_QUARANTINE_PLAN_COPY_ONLY_FIXED"
...
echo
echo "=== 1) GENERAZIONE PIANO QUARANTENA ROBUSTO ==="

{
  ...
  awk -F'\t' 'NR>1 {
    path=$2
    if (path ~ /v51r3|v51r4|v51r5|v51r6|v51r7/) {
      print "V51_RESIDUE\t" path "\tV51 patch residue not referenced in index\tCOPY_ONLY_QUARANTINE"
    }
    if (path ~ /\.bak|before_cleanup|bak_before/) {
      print "BACKUP_FILE\t" path "\tbackup/archive candidate\tCOPY_ONLY_QUARANTINE"
    }
  }' "$SRCMAP/archive_backlog.tsv"
} | awk -F'\t' 'NR==1 || !seen[$2]++' > "$PLAN"

column -t -s $'\t' "$PLAN" || cat "$PLAN"
...

{
  ...
  awk -F'\t' 'NR>1 {print $2}' "$PLAN" | while IFS= read -r p; do
    base="$(basename "$p")"
...

{
  ...
  awk -F'\t' 'NR>1 && $3=="COPIED" {print $2}' "$COPIED" | while IFS= read -r copied; do
    sha256sum "$copied"
  done
} | tee "$SHA"
...
# TRFMC Quarantine Copy-Only Fixed
...
{
  "timestamp": "$TS",
  "operation": "TRFMC_QUARANTINE_PLAN_COPY_ONLY_FIXED",
  "mutation": "copy_only",
  "delete": false,
  "move": false,
...
echo
echo "============================================================"
echo "TRFMC_QUARANTINE_PLAN_COPY_ONLY_FIXED COMPLETATO"
echo "Output: $OUT"
