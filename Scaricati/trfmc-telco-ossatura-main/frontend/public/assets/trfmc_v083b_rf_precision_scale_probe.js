(function(){
  ...
  const fragmentSrc = `
    ...
    vec3 palette(float rsrp){
      float x = clamp((rsrp + 116.0) / 48.0, 0.0, 1.0);
      ...
      vec3 hot = vec3(0.900,0.120,0.180);
    ...
    void main(){
      ...
      col = mix(col, vec3(0.0,0.012,0.040), sh*0.24);
  ...
  function addPrecisionDock(){
    ...
    document.title = "TRFMC v0.83B · RF Precision Scale + Probe Diagnostics";
    ...
    if(main && formula && !document.querySelector(".v83b-rsrp-scale")){
      ...
      scale.innerHTML = `
        ...
        <div class="v83b-scale-labels">
          ...
          <div><b>Good</b><span>-90 / -80 dBm · usable service margin</span></div>
          <div><b>Excellent</b><span>≥ -80 dBm · strong serving dominance</span></div>
        </div>
      `;
    ...
    if(main && formula && !document.querySelector(".v83b-probe-dock")){
      ...
      dock.innerHTML = `
        ...
        <article class="v83b-probe-card">
          ...
          <p>Rosso = campo forte vicino al serving site. Nero = outage/NLOS. Ciano/blu = weak/cell-edge. Verde/giallo = margine operativo utile. Questa regola rende la pagina credibile.</p>
        </article>
      `;
