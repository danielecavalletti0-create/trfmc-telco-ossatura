...
OUT="$BASE/runtime/quality/TRFMC_P2A_SIGNAL_ANALYZER_SOURCE_AUDIT_READONLY_$TS"

mkdir -p "$OUT"
cd "$BASE"

SUMMARY="$OUT/summary.json"
CANDIDATES="$OUT/p2a_signal_analyzer_candidates.tsv"
TOP="$OUT/p2a_signal_analyzer_top_candidates.tsv"
STRUCT="$OUT/p2a_signal_analyzer_structure.tsv"
KEYWORDS="$OUT/p2a_signal_analyzer_keyword_scan.tsv"
ASSETS="$OUT/p2a_signal_analyzer_asset_refs.tsv"
DEBT="$OUT/p2a_signal_analyzer_debt_scan.tsv"
HTTP="$OUT/http.tsv"
BUILDLOG="$OUT/npm_build_p2a_signal_analyzer_audit_readonly.log"
PLAN="$OUT/TRFMC_P2A_SIGNAL_ANALYZER_PROMOTION_PLAN.md"

echo "============================================================"
echo "TRFMC_P2A_SIGNAL_ANALYZER_SOURCE_AUDIT_READONLY"
echo "Read-only · Signal Analyzer candidate selection · no mutation"
echo "Timestamp: $TS"
echo "============================================================"
...
positive_name = [
    "signal", "spectrum", "analyzer", "vsa", "iq", "waterfall", "rfpro",
    "instrument", "dsp", "fft", "constellation", "source_runtime", "bridge",
    "recorder", "evidence", "flight", "sweep", "spectral"
]

negative_name = [
    "antenna", "fiber", "otdr", "core", "ran", "open5gs", "ueransim",
    "knowledge", "home", "portal_index", "integration_control", "rf_physics",
    "microwave", "wifi"
]

positive_content = [
    "spectrum", "waterfall", "fft", "iq", "i/q", "constellation", "evm",
    "obw", "aclr", "rbw", "vbw", "sweep", "trace", "span", "center frequency",
    "sample rate", "dsp", "window", "hann", "blackman", "noise floor",
    "dbm", "dbfs", "vector signal", "vsa", "modulation", "qam", "ofdm",
    "websocket", "worker", "typedarray", "canvas"
]
...
for path in sorted(public.glob("*.html")):
    ...
    if "websocket" in html_l or "ws://" in html_l:
        score += 18
        reasons.append("websocket")
    if "worker" in html_l:
        score += 14
        reasons.append("worker/dsp")
    if "fft" in html_l:
        score += 18
        reasons.append("fft")
    if "waterfall" in html_l:
        score += 18
        reasons.append("waterfall")
    if "constellation" in html_l:
        score += 15
        reasons.append("constellation")
    if "evm" in html_l:
        score += 15
        reasons.append("evm")
    if "v38" in name or "v40" in name or "v50" in name or "v60" in name:
        score += 8
...
md = []
md.append("# TRFMC P2A Signal Analyzer Source Audit")
md.append("")
md.append("## Scope")
md.append("")
md.append("Read-only audit for Signal Analyzer promotion. No source mutation, no backend mutation, no CSS patch.")
md.append("")
md.append("## Selection rule")
md.append("")
md.append("- Prefer spectrum/waterfall/IQ/FFT/VSA/EVM/canvas/WebSocket/Worker content.")
md.append("- Exclude RF Physics, Antenna, Fiber, Core/RAN and Mission Control unless only used as reference.")
...
md.append("")
md.append("`TRFMC_P2B_SIGNAL_ANALYZER_REACT_PROMOTION_V1`")
md.append("")
md.append("Required output:")
md.append("- `frontend/src/domains/signal-analyzer/SignalAnalyzerDomainP2.tsx`")
md.append("- `frontend/src/domains/signal-analyzer/signalAnalyzerRegistry.ts`")
md.append("- canvas spectrum/waterfall placeholder governed by React")
md.append("- no iframe")
md.append("- no unsafe HTML injection")
md.append("- no public HTML runtime link")
md.append("- DOM marker: `data-trfmc-p2-signal-analyzer-domain=\"mounted\"`")
...
echo
echo "=== TOP SIGNAL ANALYZER CANDIDATES ==="
...
RESULT="P2A_SIGNAL_ANALYZER_AUDIT_READY"
if [ "$CANDIDATE_COUNT" = "0" ]; then RESULT="REVIEW_NO_SIGNAL_ANALYZER_CANDIDATES"; fi
...
{
  "timestamp": "$TS",
  "operation": "TRFMC_P2A_SIGNAL_ANALYZER_SOURCE_AUDIT_READONLY",
  "mutation": false,
...
ln -sfn "$OUT" "$BASE/runtime/quality/latest_p2a_signal_analyzer_source_audit_readonly"
...
echo
echo "============================================================"
echo "TRFMC_P2A_SIGNAL_ANALYZER_SOURCE_AUDIT_READONLY COMPLETATO"
echo "Output: $OUT"
echo "============================================================"
