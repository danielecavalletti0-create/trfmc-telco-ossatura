...
QUALITY_DIR="$ROOT/runtime/quality/TRFMC_STATIC_API_GATEWAY_PACK_V21_$TS"
RELEASE_DIR="$ROOT/runtime/releases/TRFMC_STATIC_API_GATEWAY_PACK_V21_$TS"
NGINX_ROOT="$ROOT/runtime/nginx/trfmc_static_api_4181"
NGINX_CONF="$NGINX_ROOT/conf/nginx.conf"
FREEZE="$ROOT/runtime/freezes/TRFMC_STATIC_API_GATEWAY_PACK_V21_$TS.tar.gz"

echo "============================================================"
echo "TRFMC STATIC + API GATEWAY PACK V21"
echo "NGINX user-mode · static dist · optional backend proxy · JSON fallback"
echo "============================================================"
...
{
  "timestamp": "$TS",
  "operation": "TRFMC_STATIC_API_GATEWAY_PACK_V21",
  "result": "NEEDS_NGINX",
  "install_hint": "sudo apt update && sudo apt install -y nginx"
}
JSON
  ln -sfn "$QUALITY_DIR" "$ROOT/runtime/quality/latest_static_api_gateway_pack_v21"
...
echo
echo "=== CREA NGINX CONFIG USER-MODE 4181 ==="
...
http {
    ...
    server {
        ...
        location @api_backend {
            proxy_pass http://trfmc_backend_8000;
            ...
            error_page 404 500 502 503 504 = @api_offline_fallback;
        }

        location @api_offline_fallback {
            default_type application/json;
            add_header Cache-Control "no-cache";
            return 200 '{"status":"offline_or_not_implemented","source":"trfmc-nginx-v21-api-fallback","path":"\$uri","backend":"127.0.0.1:8000","mode":"safe-readonly-fallback"}';
        }
...
echo
echo "=== CREA CONTROL SCRIPT 4181 ==="

cat > "$ROOT/runtime/bin/trfmc_static_api_gateway_start_4181.sh" <<SCRIPT
#!/usr/bin/env bash
set -Eeuo pipefail
...
if curl -fsS --connect-timeout 2 --max-time 5 http://127.0.0.1:4181/ >/dev/null 2>&1; then
  echo "OK: TRFMC static API gateway already reachable at http://127.0.0.1:4181/"
  ss -ltnp | grep ':4181' || true
  exit 0
fi
...
if curl -fsS --connect-timeout 2 --max-time 5 http://127.0.0.1:4181/ >/dev/null 2>&1; then
  echo "OK: TRFMC static API gateway started"
  echo "URL: http://127.0.0.1:4181/"
  ss -ltnp | grep ':4181' || true
else
  echo "ERRORE: TRFMC static API gateway not reachable"
  tail -n 80 "\$NGINX_ROOT/logs/error.log" || true
  exit 1
fi
SCRIPT

cat > "$ROOT/runtime/bin/trfmc_static_api_gateway_stop_4181.sh" <<SCRIPT
#!/usr/bin/env bash
set -Eeuo pipefail
...
if ss -ltnp | grep -q ':4181'; then
  PID="\$(ss -ltnp | awk '/:4181/ { if (match(\$0,/pid=[0-9]+/)) { print substr(\$0,RSTART+4,RLENGTH-4); exit } }')"
  [ -n "\$PID" ] && kill "\$PID" 2>/dev/null || true
fi

echo "OK: TRFMC static API gateway stop requested"
ss -ltnp | grep ':4181' || true
SCRIPT

cat > "$ROOT/runtime/bin/trfmc_static_api_gateway_status_4181.sh" <<SCRIPT
#!/usr/bin/env bash
set -Eeuo pipefail
...
echo
echo "=== STATIC HTTP ==="
curl -I --connect-timeout 2 --max-time 6 http://127.0.0.1:4181/ || true
curl -I --connect-timeout 2 --max-time 6 http://127.0.0.1:4181/index.html || true
curl -I --connect-timeout 2 --max-time 6 http://127.0.0.1:4181/api/health || true

echo
echo "=== API FALLBACK PROBES ==="
for u in \
  /api/mission/status \
...
do
  echo
  echo "--- \$u"
  curl -sS --connect-timeout 2 --max-time 6 "http://127.0.0.1:4181\$u" | head -c 240
  echo
done

echo
echo "=== LOGS ==="
tail -n 30 "$NGINX_ROOT/logs/access.log" 2>/dev/null || true
tail -n 30 "$NGINX_ROOT/logs/error.log" 2>/dev/null || true
SCRIPT

chmod +x \
  "$ROOT/runtime/bin/trfmc_static_api_gateway_start_4181.sh" \
  "$ROOT/runtime/bin/trfmc_static_api_gateway_stop_4181.sh" \
  "$ROOT/runtime/bin/trfmc_static_api_gateway_status_4181.sh"

echo
echo "=== AVVIO API GATEWAY 4181 ==="
...
probe() {
  ...
  meta="$(curl -sS -o /dev/null -w "%{http_code}\t%{size_download}" --connect-timeout 2 --max-time 8 "http://127.0.0.1:4181$u" 2>/dev/null || printf "000\t0")"
  code="${meta%%	*}"
  bytes="${meta#*	}"
...

{
  ...
  test -x "$ROOT/runtime/bin/trfmc_static_api_gateway_start_4181.sh" && echo "OK: api gateway start script" || echo "MISS: api gateway start script"
  test -x "$ROOT/runtime/bin/trfmc_static_api_gateway_stop_4181.sh" && echo "OK: api gateway stop script" || echo "MISS: api gateway stop script"
  test -x "$ROOT/runtime/bin/trfmc_static_api_gateway_status_4181.sh" && echo "OK: api gateway status script" || echo "MISS: api gateway status script"
} > "$CONTENT_CHECK"
...
MANIFEST="$RELEASE_DIR/static_api_gateway_manifest_v21.json"
...
cat > "$MANIFEST" <<JSON
{
  ...
  "control_scripts": {
    "start": "$ROOT/runtime/bin/trfmc_static_api_gateway_start_4181.sh",
    "stop": "$ROOT/runtime/bin/trfmc_static_api_gateway_stop_4181.sh",
    "status": "$ROOT/runtime/bin/trfmc_static_api_gateway_status_4181.sh"
  },
...
tar -czf "$FREEZE" \
  ...
  runtime/nginx/trfmc_static_api_4181 \
  runtime/bin/trfmc_static_api_gateway_start_4181.sh \
  runtime/bin/trfmc_static_api_gateway_stop_4181.sh \
  runtime/bin/trfmc_static_api_gateway_status_4181.sh \
  "$RELEASE_DIR" \
  2>/dev/null || true
...
ln -sfn "$QUALITY_DIR" "$ROOT/runtime/quality/latest_static_api_gateway_pack_v21"
ln -sfn "$RELEASE_DIR" "$ROOT/runtime/releases/latest_static_api_gateway_pack_v21"
...
echo
echo "============================================================"
echo "V21 STATIC + API GATEWAY PACK COMPLETATO"
echo "URL   : http://127.0.0.1:4181/"
echo "Start : runtime/bin/trfmc_static_api_gateway_start_4181.sh"
echo "Stop  : runtime/bin/trfmc_static_api_gateway_stop_4181.sh"
echo "Status: runtime/bin/trfmc_static_api_gateway_status_4181.sh"
echo "============================================================"
