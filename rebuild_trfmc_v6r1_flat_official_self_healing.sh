...
BK="$BASE/runtime/backups/V6R1_SELF_HEAL_$TS"

mkdir -p "$PUBLIC" "$BASE/runtime/backups" "$BASE/runtime/quality" "$BK"

echo "============================================================"
echo "TRFMC V6R1 FLAT OFFICIAL - SELF HEALING LOCK"
echo "============================================================"
echo "BASE=$BASE"
echo "PUBLIC=$PUBLIC"
echo "BACKUP=$BK"
...
echo "[1/7] Backup entrypoint esistenti"
...
cp -av "$PUBLIC/trfmc_reset_browser_state_v6r1.html" "$BK/" 2>/dev/null || true

echo
echo "[2/7] Creo/Ricreo V6R1 FLAT ufficiale: iframe SOLO verso moduli foglia"
...
const WRAPPERS = {
  ...
  v1:"/trfmc_unified_instrument_shell_lab_v1.html"
};
...
function renderCards(){
  ...
  Object.entries(WRAPPERS).forEach(([k,u])=>{
    const div=document.createElement("div");
    div.className="card";
    div.onclick=()=>{window.open(u,"_blank");log("wrapper opened in new tab only → "+k.toUpperCase())};
    div.innerHTML="<b>"+k.toUpperCase()+" supervisor page</b><span>Non viene caricato dentro iframe.</span><span>"+u+"</span><small class='unk'>open only</small>";
    w.appendChild(div);
  });
}
function load(k){
...
function toggleClean(){
  ...
  log(document.body.classList.contains("clean")?"top strip hidden":"top strip visible");
}
...
function reloadFrame(){
  ...
  log("active leaf iframe reload requested");
}
function openActive(){
  window.open(LEAF[active].url,"_blank");
  log("opened standalone → "+LEAF[active].url);
}
...
echo
echo "[3/7] Creo redirect V6 -> V6R1 FLAT"
...
<title>TRFMC Official Entrypoint → V6R1 Flat</title>
<style>
...
.box{border:1px solid #1d6f9f;background:#061120;border-radius:10px;padding:24px}
h1{color:#00d9ff}
a{color:#ffd500}
</style>
</head>
<body>
<div class="box">
<h1>TRFMC Official Entrypoint</h1>
<p>Redirect alla versione corretta: V6R1 FLAT, senza iframe annidati.</p>
...
echo
echo "[4/7] Creo pagina reset stato browser"
cat > "$PUBLIC/trfmc_reset_browser_state_v6r1.html" <<'HTML'
<!doctype html>
<html lang="it">
<head>
<meta charset="utf-8">
<title>TRFMC Reset Browser State V6R1</title>
<style>
body{margin:0;background:#02070f;color:#eaf3ff;font-family:Segoe UI,system-ui,sans-serif;display:grid;place-items:center;height:100vh}
.box{border:1px solid #1d6f9f;background:#061120;border-radius:10px;padding:24px;max-width:760px}
h1{color:#00d9ff}
pre{background:#01060d;border:1px solid #245b7d;padding:12px;color:#7dff4f}
a{color:#ffd500}
</style>
</head>
<body>
...
<p>Rimuovo solo le chiavi sperimentali responsabili di layout schiacciati/kiosk persistente.</p>
<pre id="out"></pre>
<p><a href="/trfmc_official_safe_entrypoint_v6r1_flat.html">Apri V6R1 Flat</a></p>
</div>
<script>
...
const out = [];
for (const k of keys) {
  try { localStorage.removeItem(k); out.push("REMOVED: " + k); }
  catch(e) { out.push("ERROR: " + k + " " + e); }
}
document.getElementById("out").textContent = out.join("\n");
setTimeout(()=>location.href="/trfmc_official_safe_entrypoint_v6r1_flat.html", 1500);
</script>
</body>
</html>
HTML
...
echo
echo "[5/7] Creo guard no nested iframe"
cat > "$BASE/trfmc_guard_no_nested_iframe_v6r1.sh" <<'GUARD'
#!/usr/bin/env bash
set -Eeuo pipefail
...
grep -nE 'trfmc_global_instrument_shell_v1|trfmc_global_top_telemetry_v2' \
  ...
  > "$OUT/forbidden_stable_refs.txt" 2>/dev/null || true
...
echo
echo "=== FORBIDDEN STABLE REFS ==="
cat "$OUT/forbidden_stable_refs.txt"
GUARD

chmod +x "$BASE/trfmc_guard_no_nested_iframe_v6r1.sh"

echo
echo "[6/7] Lancio guard"
"$BASE/trfmc_guard_no_nested_iframe_v6r1.sh"

echo
echo "[7/7] Riepilogo URL"
echo "RESET_BROWSER=http://127.0.0.1:5173/trfmc_reset_browser_state_v6r1.html"
echo "OFFICIAL_FLAT=http://127.0.0.1:5173/trfmc_official_safe_entrypoint_v6r1_flat.html"
echo "OLD_V6_REDIRECT=http://127.0.0.1:5173/trfmc_official_safe_entrypoint_v6.html"
echo
echo "============================================================"
echo "COMPLETATO"
echo "============================================================"
